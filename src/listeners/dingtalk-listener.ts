...
export class DingTalkStreamListener {
  ...
  async start(): Promise<void> {
    ...
    this.client.on('connect', () => {
      this.connected = true
      logger.success('钉钉图片通道已连接（Stream 模式，群内发图即可自动签到）')
    })
    this.client.on('disconnect', () => {
      this.connected = false
      logger.warn('钉钉图片通道连接断开，SDK 将自动重连')
    })
    this.client.on('error', (e: any) => {
      logger.warn(`钉钉图片通道异常: ${e?.message || e}`)
    })
    ...
    try {
      ...
      logger.warn('请检查 appKey/appSecret 是否正确、机器人是否已选择 Stream 模式并发布')
    }
  }
